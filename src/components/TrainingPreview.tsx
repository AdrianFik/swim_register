...
export default function TrainingPreview({
  ...
  // Inicializar estado con el array de bloques recibido
  const [blocks, setBlocks] = useState<TrainingData[]>(data);
  const [saving, setSaving] = useState(false);
  const [success, setSuccess] = useState(false);
  
  const [pbs, setPbs] = useState<PersonalBest[]>([]);
  const [calcZoneInfos, setCalcZoneInfos] = useState<string[]>(new Array(data.length).fill(""));
  ...
  useEffect(() => {
    async function loadPbs() {
      try {
        ...
        if (res.ok) {
          const dataPbs: PersonalBest[] = await res.json();
          setPbs(dataPbs);
        }
      } catch (err) {
  ...
  // Autocalcular zona de intensidad inicial para cada bloque cuando carguen los PBs
  useEffect(() => {
    if (pbs.length === 0 || blocks.length === 0) return;
    
    const newInfos = [...calcZoneInfos];
    ...
    setCalcZoneInfos(newInfos);
  }, [pbs]);

  // Actualizar un campo de un bloque específico
  const updateField = (index: number, key: keyof TrainingData, value: string) => {
    setBlocks((prev) => {
      const next = [...prev];
  ...
  const deleteBlock = (index: number) => {
    ...
    setCalcZoneInfos((prev) => prev.filter((_, idx) => idx !== index));
  };
    ...
    return (
      <div className={styles.success}>
        ...
        <div className={styles.successSubtitle}>
          Los bloques de entrenamiento se han añadido a la hoja de{" "}
  ...
  if (blocks.length === 0) {
    return (
      <div className={styles.success}>
        <div className={styles.successTitle}>No quedan bloques</div>
        <div className={styles.successSubtitle}>
          Has descartado todos los bloques de este entrenamiento.
        </div>
        <button
          id="discard-all-empty-button"
          className={styles.newButton}
          onClick={onDiscard}
        >
          Volver a grabar
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <h2 className={styles.title}>
          Revisa los datos ({blocks.length} {blocks.length === 1 ? "bloque" : "bloques"})
        </h2>
      ...
      <div className={styles.blocksList}>
        {blocks.map((block, index) => {
          const selectedLabels = block.intensidad
            ? block.intensidad.split("+").map((s) => s.trim()).filter(Boolean)
            : [];
          ...
          const toggleLabel = (label: string) => {
            ...
            updateField(index, "intensidad", nextLabels.join(" + "));
          };

          const calcZoneInfo = calcZoneInfos[index];

          return (
            <div key={index} className={styles.card}>
              <div className={styles.cardHeader}>
                <h3 className={styles.cardTitle}>Bloque #{index + 1}</h3>
                {blocks.length > 1 && (
                  <button
                    type="button"
              ...
              {FIELDS.map((field) => (
                <div key={field.key} className={styles.field}>
                  ...
                  {field.key === "piscina" ? (
                    ...
                    <div className={styles.chipsContainer} id={`field-${index}-${field.key}`}>
